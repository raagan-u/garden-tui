...
use bitcoin::{
    ...
    Txid, Witness, CompressedPublicKey, EcdsaSighashType, PublicKey, Script,
    ScriptBuf,
};
...
pub fn initaite_htlc(priv_key_hex: &str, htlc_addr: bitcoin::Address, amount: i64, indexer_url: &str, network: bitcoin::Network) -> Result<String> {
...
pub async fn create_redeem_tx(
    ...
    let recipient = match receiver_address {
        Some(addr) => addr,
        None => get_btc_address_for_priv_key(private_key, network)?
    };
