...
pub struct Quote {
    ...
    url: String,
    strategies_map: Option<HashMap<String, Strategy>>,
}
...
impl Quote {
    ...
    pub async fn load_strategies(&mut self) -> Result<bool>{
        let url = format!("{}/quote/strategies", self.url);
        let resp = self.client.get(&url).send().await?;
        let response = resp.json::<Value>().await.map_err(|e| anyhow!("error fetching strategies from quote {}", e))?;
    ...
    pub async fn get_price(&self, order_pair: &str, amount: &str) -> Result<String> {
        let url = format!("{}/quote?order_pair={}&amount={}&exact_out={}", self.url, order_pair, amount, true);
        let resp = self.client.get(&url).send().await?;
        let response = resp.json::<Value>().await.map_err(|e| anyhow!("error getting quote price {}", e))?;
        println!("response: {:?}", response);
        Ok(response.to_string())
    }
    
    pub async fn get_attested_quote(&self, order: Order) -> Result<Order> {
